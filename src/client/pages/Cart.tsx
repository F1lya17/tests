...
export const Cart: React.FC = () => {
    ...
    if (!cartIsEmpty) {
        const rows = Object.entries(cart).map(([id, item], index) => {
            ...
            return (
                <tr role="product" key={id} data-testid={id}>
        ...
        content = (
            <>
                Cart is empty. Please select products in the <Link data-testid="empty" to="/catalog">catalog</Link>.
            </>
        );
    }
    ...
    const actions = cartIsEmpty ? null : (
        <div className="row mb-4">
            <div className="col-6">
                <button data-testid="clear" className={bem('Clear', ['btn', 'btn-outline-secondary'])} onClick={onClear}>Clear shopping cart</button>
            </div>
        </div>
    );
